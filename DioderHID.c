...
/*
	Edited 2011 by Rikard Lindström <ornotermes@gmail.com>
	This is a modified version of the GenericHID demo included in LUFA
*/
...
/*Define stuff for leds*/
#define LED_R_DDR	DDRC
#define LED_R		PC6
#define LED_R_OCR	OCR1AL

#define LED_G_DDR	DDRC
#define LED_G		PC5
#define LED_G_OCR	OCR1BL

#define LED_B_DDR	DDRB
#define LED_B		PB7
#define LED_B_OCR	OCR1CL
...
void ProcessGenericHIDReport(uint8_t* DataArray)
{
	...
	*/
	...
	if ( (DataArray[0] == 'R') || (DataArray[0] == 'r') )
	{
		LED_R_OCR = ( HexCharToN(DataArray[1])<<4 ) | ( HexCharToN(DataArray[2]) );
	}
	if ( (DataArray[0] == 'G') || (DataArray[0] == 'g') )
	{
		LED_G_OCR = ( HexCharToN(DataArray[1])<<4 ) | ( HexCharToN(DataArray[2]) );
	}
	if ( (DataArray[0] == 'B') || (DataArray[0] == 'b') )
	{
		LED_B_OCR = ( HexCharToN(DataArray[1])<<4 ) | ( HexCharToN(DataArray[2]) );
	}
	if ( (DataArray[0] == 'A') || (DataArray[0] == 'a'))
	{
		LED_R_OCR = ( HexCharToN(DataArray[1])<<4 ) | ( HexCharToN(DataArray[2]) );
		LED_G_OCR = ( HexCharToN(DataArray[3])<<4 ) | ( HexCharToN(DataArray[4]) );
